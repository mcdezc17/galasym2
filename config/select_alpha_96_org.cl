...
begin
    ...
    if(ds9_access == no){
        ...
        print("-------------------------------------------------------------")
        print(" wait... Oppening DS9")
        sleep(5)
        print(" MAKE SURE DS9 has completly opened and images are displayed")
        sleep(3)
    ...
    for(i=obj_i; i<=n_list; i+=1){
        ...
        display(image=obs_dir//"/"//"observed_"//id_obj[obj_pos], frame=2, erase=yes, >& "dev$null")
        ...
        y_0 = (ny + 1) / 2
        # color region:
        if(rot_alpha_val[radius] <= 0.095){ color_reg = "blue"}
        else if(rot_alpha_val[radius] > 0.095 && rot_alpha_val[radius] <= 0.195){ color_reg = "yellow"}
        else if(rot_alpha_val[radius] > 0.195){ color_reg = "red"}
        # crear region:
        ...
        printf(expre, rot_alpha_val[radius], >> tmp_dir//"/"//"tmp_ellipse_draw.reg")
        ...
        while (key != 100 && key != 115){
            ...
            printf("\r     |  %11.2f | %11.4f |", scale_r[radius], rot_alpha_val[radius])
            ...
            if (x > 0 || y > 0 || key > 0) {
                ...
                if (key == 43){
                    ...
                    # Imprime objeto en la lista seleccionada:
                    printf("%32s %11.4f %11.4f %11.8f %11.8f %12.2f %5.3f %11.4f\n", id_obj[obj_pos], xwin_img[obj_pos], ywin_img[obj_pos], ra_j00[obj_pos], dec_j00[obj_pos], scale_r[radius], ccdistance[obj_pos], rot_alpha_val[radius], >> rot_cat_dir//"/"//"select_rot_index.cat")
                    ...
                    printf(expre, rot_alpha_val[radius], >> ds9_dir//"/"//"select_rot_index.reg")
                    ...
                    # Imprime objeto en la lista seleccionada:
                    printf("%32s %11.4f %11.4f %11.8f %11.8f %12.2f %5.3f %11.4f\n", id_obj[obj_pos], xwin_img[obj_pos], ywin_img[obj_pos], ra_j00[obj_pos], dec_j00[obj_pos], scale_r[radius], ccdistance[obj_pos], rot_alpha_val[radius], >> data_dir//"/"//"reject_objects.cat")

                    # Crea la region ds9 para objeto descartado:
                    expre = 'ellipse('//str(ra_j00[obj_pos])//','//str(dec_j00[obj_pos])//','//str(scale_r[radius] * petro_r[obj_pos] * a_img[obj_pos] * pix_scale)//'",'//str(scale_r[radius] * petro_r[obj_pos] * b_img[obj_pos] * pix_scale)//'",'//str(theta_img[obj_pos])//') # color='//color_reg//' text={Size:'//str(scale_r[radius])//'*Rp, alpha= %.4f}\n'
                    printf(expre, rot_alpha_val[radius], >> data_dir//"/"//"reject_objects.reg")
