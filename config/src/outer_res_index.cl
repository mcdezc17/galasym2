...
begin
    ...
    printf("outer_res_index_%.1f", bulge_clip) | scan(outer_res_dir)
    ...
    for(i=1;i<=36;i+=1){
        ...
        if(i == 1){
            ...
            printf("#%31s sumF_%4.2frp", "ID_OBJ", scale_r[i], > abscat_dir//"/"//"set_flux_outer_res.cat")

            # II. PROFILE Asymmetry area SET: first
            printf("#%31s prfl_%4.2frp", "ID_OBJ", scale_r[i], > abscat_dir//"/"//"prfl_set_outer_res.cat")

            # III. CUMULATIVE Asymmetry area SET: first
            printf("#%31s cum_%4.2frp", "ID_OBJ", scale_r[i], > abscat_dir//"/"//"cum_set_outer_res.cat")
    ...
    # IV. (PROFILE CURVE)
    printf("#%31s %11s %11s %11s %11s prfl_1.0rp prfl_1.5rp prfl_2.0rp\n", "ID_OBJ", "X_IMG", "Y_IMG", "RAJ00", "DECJ00", > abscat_dir//"/"//"prfl_main_outer_res.cat")

    # V. (CUMMULATIVE CURVE)
    printf("#%31s %11s %11s %11s %11s R_A_1.0rp R_A_1.5rp R_A_2.0rp\n", "ID_OBJ", "X_IMG", "Y_IMG", "RAJ00", "DECJ00", > abscat_dir//"/"//"cum_main_outer_res.cat")
    ...
    for(i=1;i<=n_list;i+=1){
        ...
        for(j=1;j<=36;j+=1){
            ...
            if(j == 16){
                ...
                printf("%32s %11.4f %11.4f %11.8f %11.8f %11.4f", id_obj[i], x0_rot[i], y0_rot[i], ra_rot[i], dec_rot[i], outer_res_prfl_index, >> abscat_dir//"/"//"prfl_main_outer_res.cat")

                # CUMULATIVE Main catalog Asymetry
                printf("%32s %11.4f %11.4f %11.8f %11.8f %11.4f", id_obj[i], x0_rot[i], y0_rot[i], ra_rot[i], dec_rot[i], outer_res_cumm_index, >> abscat_dir//"/"//"cum_main_outer_res.cat")
                ...
                expr = 'ellipse('//str(ra_rot[i])//','//str(dec_rot[i])//','//str(1.5 * petro_r[i] * a_img[i] * pixel_scale)//'",'//str(1.5 * petro_r[i] * b_img[i] * pixel_scale)//'",'//str(theta_img[i])//') # color=red dash=1 text={'//id_obj[i]//', (1Rp): '//str(outer_res_cumm_index)//'}'
                ...
                expr = 'ellipse('//str(ra_rot[i])//','//str(dec_rot[i])//','//str(2 * petro_r[i] * a_img[i] * pixel_scale)//'",'//str(2 * petro_r[i] * b_img[i] * pixel_scale)//'",'//str(theta_img[i])//') # color=red dash=1 text={(2Rp): '//str(outer_res_cumm_index)//'}'
                print(expr, >> ds9_dir//"/"//"cum_outer_res.reg")
