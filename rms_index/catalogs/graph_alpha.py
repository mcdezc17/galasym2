...
# Nombres de archivos
nombre_archivo_1 = "rms_index/prfl_index_set.cat"
nombre_archivo_2 = "rms_index/cum_index_set.cat"
...
norm_data1 = np.max(data2) * (data1 / np.max(data1))
norm_data2 = data2 / np.max(data2)
...
plt.plot(x, norm_data1, color='black', linewidth=1, label=r'index $\alpha$')

plt.plot(x, data2, color='blue', linewidth=1, label=r'index $\alpha_{ROT}$')
#plt.plot(x, norm_data2, color='blue', linewidth=1, label=r'index $\alpha_{ROT}$')
