procedure cum_galasym2_96()

# FOR ABELL 496 TEST / default cl script name: galasym2_96.cl
# CUMULATIVE ALPHA INDEX BY ALL GALAXY (alpha(r/rp) = N_asymm(in r) / N_ttl)
# Other forms are:
...
begin
    ...
    real delta_area, nbg_noisepix, n_asymmpix, prfl_index_alpha, cum_index_alpha
...
edit_task:
    ...
    for(i = obj_i; i <= obj_f; i += 1){
        ...
        tmp_string = obs_dir//"/"//"area_obs_fix_"//id_obj[obj_pos]
        if(!imaccess(tmp_string)){

            # Total pixels (N_tot) alpha = n_asymmpix / N_tot
            imdelete(tmp_string, >& "dev$null")
            expre = "(((I-a)*cos(e) + (J-b)*sin(e))**2 / (c**2)) + (((I-a)*sin(e) - (J-b)*cos(e))**2 / (d**2)) <= 1 && f == 1 && g >= h*i"
            imexpr(expre, tmp_string, side_frame[obj_pos]/2, side_frame[obj_pos]/2, scale_r[75]*petro_r[obj_pos]*a_img[obj_pos], scale_r[75]*petro_r[obj_pos]*b_img[obj_pos], theta_img[obj_pos]*const_pi/180, mod_dir//"/"//"centermodmask_"//id_obj[obj_pos], obs_dir//"/"//"observed_"//id_obj[obj_pos], low_clip, rms_bg, dims=str(side_frame[obj_pos])//","//str(side_frame[obj_pos]), verb-)
            imstat(tmp_string, fields="mean, npix", lower=INDEF, upper=INDEF, nclip=0, format-) | scan(meanpix, ttlpix)
            n_total[obj_pos] = meanpix * ttlpix
...
rotated_index:
    ...
    for(i = obj_i; i <= obj_f; i += 1){
        ...
        for(j=1; j<=75; j+=2){
            ...
            prfl_index_alpha = (n_asymmpix - (delta_area * min_densitybg)) /  (n_total[obj_pos])
            ...
            if(j == 1){
                ...
                if(edit_mode != no){
                    ...
                    # III. Asymmetry area SET: first
                    printf("%32s %8.4f", "edit_"//id_obj[obj_pos], prfl_index_alpha, >> out_cat//"areaindex_set.cat")
                }else{
                    #       %ID  %fcor %Nt %Nasymm_1 (# I. Asymmetrical pixel SET: first)
                    ...
                    # III. Asymmetry area SET: first
                    printf("%32s %8.4f", id_obj[obj_pos], prfl_index_alpha, >> out_cat//"areaindex_set.cat")
                }
                ...
                # III. Asymmetry area SET: last
                printf(" %8.4f\n", prfl_index_alpha, >> out_cat//"areaindex_set.cat")
                ...
                # III. Asymmetry area SET: mid
                printf(" %8.4f", prfl_index_alpha, >> out_cat//"areaindex_set.cat")
            }

            # 1, 1.5 and 2 Petrosian radius output catalog (Fix apertures)
            if(j == 16){
                # MAIN CATALOG: 1.0xRp (Overwrite catalogs if edit_mode = yes)
                if(edit_mode == yes){
                    # IV. Asymetry area global with center distance
                    printf("%32s %11.8f %11.8f %5.3f %11.4f", "edit_"//id_obj[obj_pos], ra_j00[obj_pos], dec_j00[obj_pos], ccdistance[obj_pos], prfl_index_alpha, >> out_cat//"areaindex.cat")
                }else{
                    # IV. Asymetry area global with center distance
                    printf("%32s %11.8f %11.8f %5.3f %11.4f", id_obj[obj_pos], ra_j00[obj_pos], dec_j00[obj_pos], ccdistance[obj_pos], prfl_index_alpha, >> out_cat//"areaindex.cat")
                }

                # ALPHA INDEX DS9 REGION: refrence (Rp) aperture: eliptical
                expre = 'ellipse('//str(ra_j00[i])//','//str(dec_j00[i])//','//str(petro_r[i] * a_img[i] * pix_scale)//'",'//str(petro_r[i] * b_img[i] * pix_scale)//'",'//str(theta_img[i])//') # color=red dash=1 text={'//str(prfl_index_alpha)//'}'
                print(expre, >> out_ds9_cat)

            }else if(j == 26){

                # MAIN CATALOG: 1.5xRp
                printf(" %11.4f", prfl_index_alpha, >> out_cat//"areaindex.cat")

                # ALPHA INDEX DS9 REGION: measurement (1.5xRp) aperture: eliptical
                expre = 'ellipse('//str(ra_j00[i])//','//str(dec_j00[i])//','//str(1.5 * petro_r[i] * a_img[i] * pix_scale)//'",'//str(1.5 * petro_r[i] * b_img[i] * pix_scale)//'",'//str(theta_img[i])//') # color=red dash=1 text={'//str(prfl_index_alpha)//'}'
                print(expre, >> out_ds9_cat)

            }else if(j == 36){

                # MAIN CATALOG: 2xRp
                printf(" %11.4f\n", prfl_index_alpha, >> out_cat//"areaindex.cat")

                # ALPHA INDEX DS9 REGION: measurement (2xRp) aperture: eliptical
                expre = 'ellipse('//str(ra_j00[i])//','//str(dec_j00[i])//','//str(2 * petro_r[i] * a_img[i] * pix_scale)//'",'//str(2 * petro_r[i] * b_img[i] * pix_scale)//'",'//str(theta_img[i])//') # color=red dash=1 text={'//str(prfl_index_alpha)//'}'
                print(expre, >> out_ds9_cat)
